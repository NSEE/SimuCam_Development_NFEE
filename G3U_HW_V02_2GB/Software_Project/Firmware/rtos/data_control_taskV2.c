					...
					case sSubRequest:
						...
						if ( TRUE == pxDataC->bInsgestionSchedule[ucSubReqIFEE] ) {
							...
							if ( error_code == OS_NO_ERR ) {
								...
								vFTDIResetFullImage();
								bSuccess = bFTDIRequestFullImage( ucSubReqIFEE, ucSubReqICCD, ucSubCCDSide, pxDataC->usiEPn, pxDataC->xCopyNfee[ucSubReqIFEE].xCcdInfo.usiHalfWidth, ( pxDataC->xCopyNfee[ucSubReqIFEE].xCcdInfo.usiHeight + pxDataC->xCopyNfee[ucSubReqIFEE].xCcdInfo.usiOLN ) );
								if ( bSuccess == FALSE ) {
									/* Fail */
