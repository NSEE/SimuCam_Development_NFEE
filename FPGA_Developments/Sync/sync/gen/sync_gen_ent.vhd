...
-------------------------------------------------------------------------------
--
-- unit name: SYNC Module Sync Generator (sync_gen_ent)
--
--! @brief 
--
...
entity sync_gen_ent is
	...
	port(
		...
		flags_o       : out t_sync_gen_flags;
		error_o       : out t_sync_gen_error;
		sync_output_o : out std_logic_vector(1 downto 0)
		-- data bus(es)
	);
end entity sync_gen_ent;

--============================================================================
-- ! architecture declaration
--============================================================================
architecture rtl of sync_gen_ent is

	-- SYMBOLIC ENCODED state machine: s_sync_gen_STATE
	-- ===========================================================
	...
	signal s_sync_gen_state      : t_sync_gen_state; -- current state
	signal s_sync_gen_last_state : t_sync_gen_state;
	...
	signal s_registered_configs : t_sync_gen_configs;
...
begin
		...
		-- state transitions are always synchronous to the clock
			...
			case (s_sync_gen_state) is
				...
				when POLARITY_1 =>
					...
					-- default state transition
					s_sync_gen_state      <= POLARITY_1;
					s_sync_gen_last_state <= POLARITY_1;
					-- default internal signal values
					-- conditional state transition and internal signal values
					-- check if a stop command was received
					if (control_i.stop = '1') then
				...
				when STOPPED =>
					...
					if (control_i.start = '1') then
						...
						s_sync_gen_state <= s_sync_gen_last_state;
	...
	begin
		-- asynchronous reset
		if (reset_n_i = '0') then
			-- output generation when s_sync_gen_state changes
			sync_output_o(1) <= '0';
