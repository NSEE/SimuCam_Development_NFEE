-- MebX_Qsys_Project_ext_flash.vhd
...
entity MebX_Qsys_Project_ext_flash is
	generic (
		TCM_ADDRESS_W                  : integer := 26;
		TCM_DATA_W                     : integer := 16;
		TCM_BYTEENABLE_W               : integer := 2;
		TCM_READ_WAIT                  : integer := 100;
		TCM_WRITE_WAIT                 : integer := 100;
	...
	port (
		...
		uas_address          : in  std_logic_vector(25 downto 0) := (others => '0'); --   uas.address
...
end entity MebX_Qsys_Project_ext_flash;

architecture rtl of MebX_Qsys_Project_ext_flash is
	component altera_tristate_controller_translator is
		...
		port (
			...
			s0_uav_address       : in  std_logic_vector(25 downto 0) := (others => 'X'); -- address
			...
			m0_uav_address       : out std_logic_vector(25 downto 0);                    -- address
	...
	component altera_tristate_controller_aggregator is
		...
		port (
			...
			tcm0_address           : out std_logic_vector(25 downto 0);                    -- address_out
	...
	signal tdt_avalon_universal_master_0_address           : std_logic_vector(25 downto 0); -- tdt:m0_uav_address -> slave_translator:uav_address
	...
	signal slave_translator_avalon_anti_slave_0_address    : std_logic_vector(25 downto 0); -- slave_translator:av_address -> tda:av_address
...
begin
	...
	tcm_address_w_check : if TCM_ADDRESS_W /= 26 generate
	...
	tcm_read_wait_check : if TCM_READ_WAIT /= 100 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;

	tcm_write_wait_check : if TCM_WRITE_WAIT /= 100 generate
		assert false report "Supplied generics do not match expected generics" severity Failure;
	end generate;
...
end architecture rtl; -- of MebX_Qsys_Project_ext_flash
